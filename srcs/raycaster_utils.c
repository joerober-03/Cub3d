...
void	find_hit(t_var *v)
{
	while (v->ray.hit == 0)
	{
		...
		if (v->ray.map_x >= v->map.map_height || v->ray.map_x < 0
			|| v->ray.map_y >= v->map.map_width || v->ray.map_y < 0)
			break ;
		if (v->map.map[v->ray.map_x][v->ray.map_y] == 1)
			v->ray.hit = 1;
	}
}
