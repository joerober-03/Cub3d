...
int	move1(t_var *v, int keycode)
{
	if (keycode == 13)
	{
		if (v->map.map[(int)(v->pos.pos_x + v->pos.dir_x * 0.5)]
				[(int)v->pos.pos_y] == 0
				&& (v->pos.pos_x + v->pos.dir_x) > 0)
			v->pos.pos_x += v->pos.dir_x * 0.2;
		if (v->map.map[(int) v->pos.pos_x]
			[(int)(v->pos.pos_y + v->pos.dir_y * 0.5)] == 0
			&& (v->pos.pos_y + v->pos.dir_y) > 0)
			v->pos.pos_y += v->pos.dir_y * 0.2;
		return (1);
	}
	else if (keycode == 0)
	{
		if (v->map.map[(int)(v->pos.pos_x - v->pos.dir_y * 0.5)]
				[(int)v->pos.pos_y] == 0
				&& (v->pos.pos_x - v->pos.dir_y) > 0)
			v->pos.pos_x -= v->pos.dir_y * 0.1;
		if (v->map.map[(int)v->pos.pos_x]
			[(int)(v->pos.pos_y + v->pos.dir_x * 0.5)] == 0
			&& (v->pos.pos_y + v->pos.dir_x) > 0)
